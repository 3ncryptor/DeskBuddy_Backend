...
const allowedStages = {
  ...
  documents: ["documentsVerified", "documentsVerifiedBy", "documentsVerifiedTime"],
...
const scanArrival = async (req, res) => {
  ...
  if (!studentId || !volunteerName) {
    logger.warn("Missing required fields for arrival scan", { studentId, volunteerName });
    logger.api.error("POST", "/api/scan/arrival", "Missing required fields", { studentId, volunteerName });
    return res.status(400).json({ error: "studentId and volunteerName are required" });
  }
  ...
  try {
    ...
    if (fetchError || !student) {
      ...
      logger.scan.error("arrival", studentId, fetchError || "Student not found", { volunteerName });
      logger.api.error("POST", "/api/scan/arrival", fetchError || "Student not found", { studentId, volunteerName });
    ...
    if (student.arrival) {
      ...
      logger.scan.error("arrival", studentId, "Already arrived", { volunteerName });
    ...
    if (updateError) {
      ...
      logger.api.error("POST", "/api/scan/arrival", updateError, { studentId, volunteerName });
    ...
    logger.api.error("POST", "/api/scan/arrival", err, { studentId, volunteerName });
...
const scanHostel = async (req, res) => {
    ...
    try {
      ...
      if (fetchError || !student) {
        logger.error("Student not found", { studentId, error: fetchError?.message });
        return res.status(404).json({ error: "Student not found" });
      }
  
      if (student.hostelVerified) {
        logger.warn("Student already hostel verified", { studentId, hostelVerifiedTime: student.hostelVerifiedTime });
        return res.status(409).json({ 
      ...
      if (updateError) {
        logger.error("Failed to update hostel status", { studentId, error: updateError.message });
        return res.status(500).json({ error: "Failed to update hostel status" });
      }
  
      logger.info("Hostel scan successful", { studentId, volunteerName, hostelVerifiedTime: currentTime });
      return res.status(200).json({ message: "Hostel verification recorded", student: updatedStudent });
  ...
  const scanDocuments = async (req, res) => {
    ...
    try {
      ...
      if (student.documentsVerified) {
        logger.warn("Student documents already verified", { studentId, documentsVerifiedTime: student.documentsVerifiedTime });
        return res.status(409).json({ 
      ...
      if (updateError) {
        logger.error("Failed to update document status", { studentId, error: updateError.message });
        return res.status(500).json({ error: "Failed to update document status" });
      }
  
      logger.info("Document scan successful", { studentId, volunteerName, documentsVerifiedTime: currentTime });
      return res.status(200).json({ message: "Documents verification recorded", student: updatedStudent });
  ...
  const scanKit = async (req, res) => {
    ...
    try {
      ...
      if (student.kitReceived) {
        logger.warn("Student already received kit", { studentId, kitReceivedTime: student.kitReceivedTime });
        return res.status(409).json({ 
      ...
      if (updateError) {
        logger.error("Failed to update kit status", { studentId, error: updateError.message });
        return res.status(500).json({ error: "Failed to update kit status" });
      }
  
      logger.info("Kit scan successful", { studentId, volunteerName, kitReceivedTime: currentTime });
      return res.status(200).json({ message: "Kit delivery recorded", student: updatedStudent });
